...
async function seed() {
  if (!USER_ID) {
    ...
    console.log("Usage: SEED_USER_ID=your-user-id pnpm run seed");
    process.exit(1);
  }
  ...
  try {
    ...
    if (!board) {
      ...
      const { initializeUserBoard } =
        await import("@/lib/database/functions/init-board-user");
      // initializeUserBoard returns a Result, not the document itself.
      const { data, error } = await initializeUserBoard(USER_ID);
